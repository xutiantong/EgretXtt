...
var cores = (function (_super) {
    __extends(cores, _super);
    function cores() {
        var _this = _super.call(this) || this;
        _this.dg = DRAGONBONES.getinstance();
        _this.pr = PARTICLE.getinstance();
        _this.index = new z.index();
        _this.changjing2 = new changjing2(_this.index);
        _this.addChild(_this.changjing2);
        _this.changjing2.to(_this.index);
        return _this;
    }
    /**
     * 随机数组内元素顺序
     */
    cores.prototype.suijishunxubyarr = function (arr) {
     ...
     * num:要几个随机元素
     */
    cores.prototype.suiji = function (arr, num) {
        var arrold = new Array();
        arrold = arr.slice();
        var arrnew = new Array();
        for (var i = 0; i < num; i++) {
            var rodow = cores.getinstatic().getsuiji(0, arrold.length - 1); //产生随机数用作下标 
     ...
     * max:最大个数
     */
    cores.prototype.suijifanwei = function (arr, min, max) {
        var arrold = new Array();
        var arrnew = new Array();
        arrold = arr.slice();
     ...
     * 获取范围内随机整数
     */
    cores.prototype.getsuiji = function (min, max) {
        return Math.floor(Math.random() * (max - min + 1) + min);
    };
    /**
     * 获取范围内随机小数
     */
    cores.prototype.getsuijixiaoshu = function (min, max) {
        return (Math.random() * (max - min + 1) + min);
    };
    /**
     * 去掉滤镜
     */
    cores.prototype.remfilter = function (img) {
        img.filters = [];
    };
    /**
     * 任意颜色滤镜
     */
    cores.prototype.lvjin_any = function (an, colori) {
     ...
     * 添加滤镜(红绿)
     */
    cores.prototype.lvjin_red_green = function (img, boo) {
        var ss;
        if (boo == false) {
            ss = 0xF92727;
        }
...
__reflect(cores.prototype, "cores", ["eui.UIComponent", "egret.DisplayObject"]);
