class GlobalManager extends eui.Component implements eui.UIComponent {
	private static st: GlobalManager = null;//单例存储
	public dg: DragonBonesManager;//龙骨调用类
	mainscene: MainScene;

	constructor() {
		super();
		this.dg = DragonBonesManager.getInstance();
	...
	public suiji(arr: Array<any>, num: number) {
		...
		for (let i = 0; i < num; i++) {
			let rodow = GlobalManager.getInstance().getsuiji(0, arrold.length - 1);//产生随机数用作下标 
