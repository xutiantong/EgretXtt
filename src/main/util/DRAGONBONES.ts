...
class DRAGONBONES {
     ...
     * png：纹理集
     */
    public addToFactory(ske: string, json: string, png: string) {
        this.egretFactory.parseDragonBonesData(RES.getRes(ske));
        this.egretFactory.parseTextureAtlasData(RES.getRes(json), RES.getRes(png));
     ...
     * 
     */
    public playAnimation(armaturename: string, animationname: string, groupname: string, com: any, playtime: number, scaleX?: number, scaleY?: number, speed?: number, maskboundname?: string, layer?: number): dragonBones.AnimationState {
        this.dragonBones_arr.forEach((v, i) => {
            if (armaturename == v.name) {
                let armature: dragonBones.EgretArmatureDisplay = v.gujia;
             armature.anchorOffsetX=0
     ...
     */
    public changeslotimg(bei_armaturename: string, bei_slotname: string, bei_slotdisplayname: string, forarmaturename_nicheng: string, forslotname: string) {
        let b;
        this.dragonBones_arr.forEach((v, i) => {
